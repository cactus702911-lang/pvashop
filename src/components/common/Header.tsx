...
const Header: React.FC = () => {
  ...
  const navLinks = [
    ...
    // Shop is now part of categories or separate? Let's keep Shop as "All Products"
    { name: 'Shop', path: '/shop' },
    { name: 'Blog', path: '/blog' },
      ...
      {isMenuOpen && (
        <div className="md:hidden absolute top-16 left-0 w-full bg-white border-b border-gray-100 shadow-lg animate-in slide-in-from-top-5 max-h-[80vh] overflow-y-auto">
          <div className="container mx-auto px-4 py-4 flex flex-col gap-4">
            <Link to="/" className={`text-base py-2 ${isActive('/')}`} onClick={() => setIsMenuOpen(false)}>Home</Link>
            ...
            <Link to="/services" className={`text-base py-2 ${isActive('/services')}`} onClick={() => setIsMenuOpen(false)}>Services</Link>
            <Link to="/about" className={`text-base py-2 ${isActive('/about')}`} onClick={() => setIsMenuOpen(false)}>About</Link>
            <Link to="/blog" className={`text-base py-2 ${isActive('/blog')}`} onClick={() => setIsMenuOpen(false)}>Blog</Link>
            <Link to="/contact" className={`text-base py-2 ${isActive('/contact')}`} onClick={() => setIsMenuOpen(false)}>Contact</Link>
